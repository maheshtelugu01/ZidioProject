...
public class PaymentServiceImpl implements IPaymentService {
	...
	public String makePayment(PaymentDto dto)throws Exception {
		...
		template.convertAndSend("notification-topic",new NotificationDto(email,"Your Payment Was '"+paymentStatus+"' and Subscription Staus is '"+status+"' ","PAYMENT-SERVICE"));
	...
	public List<PaymentDto> getAllPayments() {
		String email=userClient.findUserEmail();
		return paymentRepo.findByEmail(email).stream().map(pay -> mapper.map(pay, PaymentDto.class))
